...
void tabulate(void)
{
    // updates the votes of each candidates
...
bool print_winner(void)
{
    // checks votes of each candidates and prints the winner
...
bool is_tie(int min)
{
    // TO check if theres a tie between any two candidates
    int tieCount = 0;
    int onRun = 0;
...
void eliminate(int min)
{
    //loop checks for the min value and eliminates it
    for (int c = 0; c < candidate_count; c++)
    {
        if (candidates[c].eliminated == false && candidates[c].votes == min)
        {
